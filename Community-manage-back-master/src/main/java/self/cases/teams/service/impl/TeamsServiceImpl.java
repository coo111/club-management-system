...
public class TeamsServiceImpl implements TeamsService {
    ...
    public void update(Teams teams) {
        ...
        if(StringUtils.isNotNullOrEmpty(teams.getManager()) && (!teams.getManager().equals(oldTeam.getManager()))){

            // 修改原团长身份并从社团中移除
            Users oldManage = usersDao.selectById(oldTeam.getManager());
            oldManage.setType(2);
            usersDao.updateById(oldManage);
            QueryWrapper<Members> qw_members = new QueryWrapper<Members>();
            qw_members.eq("team_id", teams.getId());
            qw_members.eq("user_id", oldManage.getId());
            membersDao.delete(qw_members);


            // 更改新团长身份并添加到社团中
            Users newManage = usersDao.selectById(teams.getManager());
            newManage.setType(1);
            usersDao.updateById(newManage);
            Members member = new Members();
            member.setId(IDUtils.makeIDByCurrent());
            member.setUserId(newManage.getId());
            member.setTeamId(teams.getId());
            member.setCreateTime(DateUtils.getNowDate());
            membersDao.insert(member);
    ...
    public Boolean isCanManage(String userId){
        ...
        QueryWrapper<Members> qw_members = new QueryWrapper<Members>();
        qw_members.eq("user_id", userId);
        if(membersDao.selectCount(qw_members) > 0){

            return false;
        }

        QueryWrapper<Teams> qw_team = new QueryWrapper<Teams>();
        qw_team.eq("manager", userId);
        if(teamsDao.selectCount(qw_team) > 0){
    ...
    public PageData parsePage(Page<Teams> p) {
        ...
        for (Teams teams : p.getRecords()) {
            ...
            temp.put("managerName", teams.getManagerName());
