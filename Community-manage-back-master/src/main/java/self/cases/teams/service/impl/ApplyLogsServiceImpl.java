...
public class ApplyLogsServiceImpl implements ApplyLogsService {
    ...
    public Boolean isApply(String userId, String teamId){
        ...
        QueryWrapper<Members> qw_member = new QueryWrapper<Members>();
        qw_member.eq("team_id", teamId);
        qw_member.eq("user_id", userId);

        if(membersDao.selectCount(qw_member) > 0){
        ...
        if(applyLogsDao.selectCount(qw) > 0){
